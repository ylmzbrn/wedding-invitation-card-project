  ...
  return (
    <div className="relative flex items-center justify-center min-h-screen overflow-hidden bg-[#fdfaf5] p-4 select-none">
      ...
      <div className="relative w-full max-w-[420px] h-[650px] flex items-center justify-center">
        ...
        <div className={`relative w-full h-[260px] transition-all duration-1000 ease-in-out ${isOpen ? "translate-y-80 opacity-0 pointer-events-none" : "z-40 animate-subtle-shake"}`}>
          <div className="relative w-full h-full bg-white shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-gray-50 rounded-sm">
            <div className="absolute top-0 left-0 w-full h-full bg-[#fcfcfc] shadow-sm z-40 origin-top transition-transform duration-700"
              style={{ clipPath: "polygon(0 0, 100% 0, 50% 60%)", borderBottom: "1px solid rgba(0,0,0,0.02)", transform: isOpen ? "rotateX(160deg)" : "rotateX(0deg)" }}>
            </div>
            <button onClick={() => setIsOpen(true)} className="absolute top-[52%] left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 transition-all hover:scale-110 active:scale-95">
            ...
            <div className="absolute inset-0 z-30 pointer-events-none" style={{ clipPath: "polygon(0 0, 0% 100%, 100% 100%, 100% 0, 50% 60%)", background: "#fff" }}>
        ...
        {/* DAVETİYE KARTI */}
        <div className={`absolute left-[2%] w-[96%] min-h-[560px] bg-white p-8 shadow-[0_30px_70px_rgba(94,11,21,0.2)] border border-[#d4af37]/15 transition-all duration-1000 delay-200 ease-out ${isOpen ? "top-2 opacity-100 scale-100 z-30" : "top-40 opacity-0 scale-95 z-20"}`}>
          <div className="border border-[#d4af37]/20 p-6 text-center h-full flex flex-col justify-between space-y-4 bg-[#fffdfa]">
            ...
            <div className="mt-1 px-2">
              <button onClick={() => window.open('https://maps.app.goo.gl/LbucjBD9dURqeQBm9', '_blank')} className="w-full py-3 border border-[#5e0b15]/20 text-[#5e0b15] text-[10px] tracking-[0.3em] hover:bg-[#5e0b15]/5 transition-all rounded-sm uppercase flex items-center justify-center gap-2 shadow-sm">
              ...
              {rsvpStatus === null ? (
                <>
                  ...
                  <div className="flex flex-col gap-2.5">
                    <button onClick={() => handleRSVP("EVET")} className="w-full py-4 bg-[#5e0b15] text-white text-[11px] tracking-[0.2em] hover:bg-[#900c3f] transition-all rounded-sm uppercase shadow-lg active:scale-[0.97] font-medium">Katılıyorum</button>
                    <button onClick={() => handleRSVP("HAYIR")} className="w-full py-4 border border-gray-100 text-gray-400 text-[11px] tracking-[0.2em] hover:bg-gray-50 transition-all rounded-sm uppercase font-medium active:scale-[0.97]">Katılamıyorum</button>
                  </div>
                </>
              ) : (
                <div className="py-2 animate-in fade-in zoom-in duration-500 space-y-4">
                  {rsvpStatus === "EVET" ? (
                    <div className="space-y-4">
                      ...
                      <div className="bg-[#5e0b15]/5 p-4 rounded-sm border border-dashed border-[#5e0b15]/20">
                        <p className="text-[10px] text-gray-500 uppercase tracking-widest mb-3 leading-relaxed">Bu mutlu günü ölümsüzleştirdiğiniz fotoğrafları bizlerle paylaşın:</p>
                        <label className="cursor-pointer block">
                          <span className="bg-white border border-[#5e0b15]/20 text-[#5e0b15] text-[9px] py-2 px-4 rounded-sm uppercase tracking-widest hover:bg-[#5e0b15] hover:text-white transition-all inline-block">Fotoğraf Yükle</span>
