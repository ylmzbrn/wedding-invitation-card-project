...
export default function Envelope({ guestName }: { guestName: string }) {
  ...
  // Renk Paleti: Daha Canlı ve Belirgin Bordo/Pembe Tonları
  const heartColors = [
    '#5e0b15', // Klasik Koyu Bordo
    '#900c3f', // Canlı Bordo/Magenta
    '#c70039', // Parlak Kırmızı/Bordo
    '#ff573388', // Yarı Saydam Turuncu/Pembe
    '#5e0b15cc' // Opak Koyu Bordo
  ];

  return (
    <div className="relative flex items-center justify-center min-h-screen overflow-hidden bg-[#fdfaf5] p-4 select-none">
      
      {/* BELİRGİNLEŞTİRİLMİŞ ARKA PLAN DEGRADELERİ */}
      {/* Sol üstteki pembe/bordo parıltı daha opak hale getirildi */}
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_30%,_rgba(144,12,63,0.3)_0%,_transparent_60%)]"></div>
      {/* Sağ alttaki koyu bordo parıltı daha opak hale getirildi */}
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_80%_70%,_rgba(94,11,21,0.25)_0%,_transparent_60%)]"></div>

      {/* YÜZEN KALPLER (AYNI KALDI) */}
        ...
        {mounted && [...Array(30)].map((_, i) => (
          <div
            ...
            style={{
              ...
              opacity: 0.15, // Hafifçe artırıldı
      ...
      <div className="relative w-full max-w-[420px] h-[650px] flex items-center justify-center">
        ...
        {/* ZARF GRUBU */}
        <div className={`relative w-full h-[260px] transition-all duration-1000 ease-in-out
        ...
        {/* DAVETİYE KARTI */}
        <div className={`absolute left-[2%] w-[96%] min-h-[560px] bg-white p-8 shadow-[0_30px_70px_rgba(94,11,21,0.2)] border border-[#d4af37]/15 transition-all duration-1000 delay-200 ease-out
          ${isOpen ? 'top-2 opacity-100 scale-100 z-30' : 'top-40 opacity-0 scale-95 z-20'}`}>
          
          <div className="border border-[#d4af37]/20 p-6 text-center h-full flex flex-col justify-between space-y-8 bg-[#fffdfa]">
              ...
              <div className="flex flex-col gap-2.5">
                {rsvpStatus === null ? (
                  ...
                  <div className="py-6 animate-in fade-in zoom-in duration-500">
                    ...
                    <button onClick={() => setRsvpStatus(null)} className="mt-4 text-[9px] text-gray-400 underline tracking-widest uppercase">Yanıtı Değiştir</button>
                  </div>
                )}
              </div>
            </div>
