    ...
    HTML5HistoryManager.prototype.addHistory = function (url, replace) {
        ...
        if (!this.disabled && (((_a = window.history.state) === null || _a === void 0 ? void 0 : _a.navigationLink) || this.getUrl(window.location)) !== url) {
            if (!replace)
                ...
                window.history.replaceState(__assign(__assign({}, window.history.state), { navigationLink: url }), null, this.getHref(url));
        }
    };
    ...
    Segment.prototype.parse = function (defaults) {
        ...
        for (var i = 0; i < matches.length; i++) {
            ...
            if (subSegment.slice(0, 1) === '{') {
                ...
                this.pattern += !this.optional ? "(".concat(subPattern, ")") : "(/".concat(subPattern, ")");
...
var StateNavigator = /** @class */ (function () {
    ...
    StateNavigator.prototype.createStateContext = function (state, data, hash, crumbs, url, asyncData, history, historyAction, currentContext) {
    ...
    StateNavigator.prototype.navigateLink = function (url, historyAction, history, suspendNavigation, currentContext) {
        ...
        var navigateContinuation = function (asyncData) {
            var nextContext = _this.createStateContext(state, data, hash, crumbs, url, asyncData, history, historyAction, currentContext);
            if (context === _this.stateContext) {
                suspendNavigation(nextContext, function () {
                    if (context === _this.stateContext)
                        _this.resumeNavigation(nextContext);
                });
            }
        };
    ...
    StateNavigator.prototype.resumeNavigation = function (stateContext) {
        this.stateContext = stateContext;
        var oldState = stateContext.oldState, state = stateContext.state, data = stateContext.data, asyncData = stateContext.asyncData, url = stateContext.url, crumbs = stateContext.crumbs, historyAction = stateContext.historyAction;
        this.rewriteCache.rewrites = {};
